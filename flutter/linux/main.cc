...
bool flutter_hbbdesk_core_main() {
   ...
   auto core_main = (FoxxDeskCoreMain) dlsym(librustdesk,"foxxdesk_core_main");
   char* error;
   if ((error = dlerror()) != nullptr) {
       fprintf(stderr, "Program entry \"foxxdesk_core_main\" is not found: %s\n", error);
       return false;
   }
   return core_main();
}
