...
static const char* kSideButtonChannelName = "org.foxxdesk.foxxdesk/side_buttons";
...
static void my_application_activate(GApplication* application) {
  ...
  // try setting icon for foxxdesk, which uses the system cache
  GtkIconTheme* theme = gtk_icon_theme_get_default();
  gint icons[4] = {256, 128, 64, 32};
  for (int i = 0; i < 4; i++) {
    GdkPixbuf* icon = gtk_icon_theme_load_icon(theme, "foxxdesk", icons[i], GTK_ICON_LOOKUP_NO_SVG, NULL);
    if (icon != nullptr) {
  ...
  if (use_header_bar) {
    ...
    gtk_header_bar_set_title(header_bar, "foxxdesk");
    gtk_header_bar_set_show_close_button(header_bar, TRUE);
    gtk_window_set_titlebar(window, GTK_WIDGET(header_bar));
  } else {
    gtk_window_set_title(window, "foxxdesk");
  }
