...
class MainFlutterWindow: NSWindow {
    ...
    private func enableNativeRelativeMouseMode(channel: FlutterMethodChannel) -> Bool {
        ...
        if result != CGError.success {
            NSLog("[FoxxDesk] Failed to dissociate mouse from cursor position: %d", result.rawValue)
            return false
        }
        ...
        if state.eventMonitor == nil {
            NSLog("[FoxxDesk] Failed to create event monitor for relative mouse mode")
    ...
    private func disableNativeRelativeMouseMode() {
        ...
        if result != CGError.success {
            NSLog("[FoxxDesk] Failed to re-associate mouse with cursor position: %d, scheduling retry...", result.rawValue)
            ...
            DispatchQueue.main.asyncAfter(deadline: .now() + 0.05) {
                ...
                if retryResult != CGError.success {
                    NSLog("[FoxxDesk] Retry failed to re-associate mouse: %d. Cursor may remain locked.", retryResult.rawValue)
                }
            }
        }
    }
    ...
    public func setMethodHandler(registrar: FlutterPluginRegistrar) {
        let channel = FlutterMethodChannel(name: "org.foxxdesk.foxxdesk/host", binaryMessenger: registrar.messenger)
