...
mod tests {
    ...
    fn test_executable_paths_match_windows_normalization() {
        let left = std::path::PathBuf::from(r"\\?\C:\Program Files\FoxxDesk\FoxxDesk.exe");
        let right = std::path::PathBuf::from(r"c:\program files\foxxdesk\foxxdesk.exe");
    ...
    fn test_os_str_eq_ignore_ascii_case_for_process_names() {
        assert!(super::os_str_eq_ignore_ascii_case(
            Some(std::ffi::OsStr::new("FoxxDesk")),
            Some(std::ffi::OsStr::new("foxxdesk"))
        ));
        assert!(!super::os_str_eq_ignore_ascii_case(
            Some(std::ffi::OsStr::new("FoxxDesk")),
