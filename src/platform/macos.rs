...
// remove just one app from all the permissions: tccutil reset All com.carriez.foxxdesk
...
fn correct_app_name(s: &str) -> String {
    ...
    if let Some(bundleid) = get_bundle_id() {
        s = s.replace("com.carriez.foxxdesk", &bundleid);
    }
    s = s.replace("foxxdesk", &crate::get_app_name().to_lowercase());
    s = s.replace("FoxxDesk", &crate::get_app_name());
    s
}
